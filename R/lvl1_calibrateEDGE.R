...
lvl1_calibrateEDGE <- function(prices, GCAM_data, logit_exp_data, vot_data, price_nonmot){
    share <- tech_output <- tot_VOT_price <- fuel_price_pkm <- non_fuel_price <- logit.exponent <- iso <- vehicle_type <- technology <- sw <- `.` <- subsector_L1 <- subsector_L2 <- subsector_L3 <- sector <- V1 <- V2 <- time_price <- tot_price <- NULL
    ##==== functions ====
    ...
    addmissingSW=function(calibr_demand,grouping_value,df_sw){
      ...
      df_sw=rbind(tmp,df_sw)
      return(df_sw)
    }
    
    ## historical values of the demand, fuel level
    calibr_demand=GCAM_data[["tech_output"]]
    ...
    base_SW=base_SW[,share:=tech_output/sum(tech_output),by=c("iso","year","vehicle_type")]
    ...
    price_FV= merge(price_FV, price_nonmot,all=TRUE, by=c("tot_price","iso","year","technology","vehicle_type","subsector_L1","subsector_L2","subsector_L3","sector"))
    ...
    dups <- duplicated(price_FV, by=c("iso", "technology", "vehicle_type","year"))
    
    if(any(dups)){
      warning("Duplicated techs found in supplied demand.")
      print(price_FV[dups])
      price_FV <- unique(price_FV, by=c("iso", "technology", "vehicle_type","year"))
    }
    

    FV_SW=merge(price_FV,base_SW,all=FALSE,by=c("iso","year","technology","vehicle_type","subsector_L1","subsector_L2","subsector_L3","sector"))
    ...
    FV_SW[iso %in% c("ARM","AZE","GEO","KAZ","KGZ","MNG","TJK","TKM","UZB") & vehicle_type=="Motorcycle (50-250cc)" & technology=="LA-BEV",
          sw:=0]

    FV_SW[iso %in% c("ARM","AZE","GEO","KAZ","KGZ","MNG","TJK","TKM","UZB") & vehicle_type=="Motorcycle (50-250cc)" & technology=="Liquids",
          sw:=1]

    
    ## reshape, save and store the sw at this level
    FV_final_SW=FV_SW[,.(iso,year,technology,vehicle_type,subsector_L1,subsector_L2,subsector_L3,sector,sw)]

    ## all technologies that have 0 tech_output_organised3 have to be included before assuming the SW trend:
    ## some technologies need to start with a sw=0 (e.g. Mini Car FCEV: it has alternatives-Liquids, NG...-)
    tmp=copy(calibr_demand)
    ...
    ## merge to the "normally calculated" sw, and create the output at this level
    FV_final_SW=rbind(FV_final_SW,tmp)

    ## merge value of time and assign 0 to the entries that don't have it
    FV_SW=merge(FV_SW,value_time_FV,all.x=TRUE,by=c("iso", "year", "vehicle_type","subsector_L1"))
    FV_SW[,time_price:=ifelse(is.na(time_price),0,time_price)]
    FV_SW[,tot_price:=tot_price+time_price]

    ## calculate price of one level up
    FV_SW=FV_SW[,.(tot_price=sum(share*tot_price),tech_output=sum(tech_output)),by = .(iso,year,vehicle_type,subsector_L1,subsector_L2,subsector_L3,sector)]
    ...
    VS1_SW[,share:=tech_output/sum(tech_output),by=c("iso","year","subsector_L1")]
    ## optimize
    VS1_SW=sw_calc(df_sw = VS1_SW, exp_prices = c(1,2,6,5,3,2),grouping_value = "subsector_L1")
    
    ## reshape, save and store the sw at this level
    VS1_final_SW=VS1_SW[,.(iso,year,vehicle_type,subsector_L1,subsector_L2,subsector_L3,sector,sw)]
    ## attribute SW==0 to the missing entries
    VS1_final_SW=addmissingSW(calibr_demand = calibr_demand,grouping_value = "vehicle_type",df_sw = VS1_final_SW)
    
    ## merge value of time and assign 0 to the entries that don't have it
    VS1_SW=merge(VS1_SW,value_time_VS1,all.x=TRUE,by=c("iso", "year", "vehicle_type", "subsector_L1"))
    VS1_SW[,time_price:=ifelse(is.na(time_price),0,time_price)]
    VS1_SW[,tot_price:=tot_price+time_price]

    VS1_SW=VS1_SW[,.(tot_price=sum(share*tot_price),tech_output=sum(tech_output)),by = .(iso,year,subsector_L1,subsector_L2,subsector_L3,sector)]

    ## rename the database, now it's going from V to S1
    S1S2_sw=merge(VS1_SW,logit_exponent_S1S2,all.x=TRUE)
    S1S2_sw[,share:=tech_output/sum(tech_output),by=c("iso","year","sector","subsector_L3","subsector_L2")]
    ...
    ## reshape, save and store the sw at this level
    S1S2_final_SW=S1S2_sw[,.(iso,year,subsector_L1,subsector_L2,subsector_L3,sector,sw)]
    ## attribute SW==0 to the missing entries
    S1S2_final_SW=addmissingSW(calibr_demand = calibr_demand,grouping_value = "subsector_L1",df_sw = S1S2_final_SW)

    S1S2_sw=merge(S1S2_sw,value_time_S1S2,all.x = TRUE,by = c("iso", "year", "subsector_L1","subsector_L2"))
    S1S2_sw[,time_price:=ifelse(is.na(time_price),0,time_price)]
    S1S2_sw[,tot_price:=tot_price+time_price]

    S1S2_sw=S1S2_sw[,.(tot_price=sum(share*tot_price),tech_output=sum(tech_output)),by = .(iso,year,subsector_L2,subsector_L3,sector)]

    ## rename the database, now it's going from S2 to S3
    S2S3_sw=merge(S1S2_sw,logit_exponent_S2S3,all.x=TRUE)
    S2S3_sw[,share:=tech_output/sum(tech_output),by=c("iso","year","sector","subsector_L3")]
    ...
    ## reshape, save and store the sw at this level
    S2S3_final_SW=S2S3_sw[,.(iso,year,subsector_L2,subsector_L3,sector,sw)]
    ## attribute SW==0 to the missing entries    
    S2S3_final_SW=addmissingSW(calibr_demand = calibr_demand, grouping_value = "subsector_L2", df_sw = S2S3_final_SW)

    S2S3_sw=merge(S2S3_sw,value_time_S2S3,all.x = TRUE,by = c("iso", "year", "subsector_L2","subsector_L3"))
    S2S3_sw[,time_price:=ifelse(is.na(time_price),0,time_price)]
    S2S3_sw[,tot_price:=tot_price+time_price]

    S2S3_sw=S2S3_sw[,.(tot_price=sum(share*tot_price),tech_output=sum(tech_output)),by = .(iso,year,subsector_L3,sector)]

    ## rename the database, now it's going from S2 to S3
    S3S_sw=merge(S2S3_sw,logit_exponent_S3S,all.x=TRUE)
    S3S_sw[,share:=tech_output/sum(tech_output),by=c("iso","year","sector")]
    
    S3S_sw=sw_calc(df_sw = S3S_sw, exp_prices = c(2,5,3,1,4), grouping_value = "sector")

    ## reshape, save and store the sw at this level
    S3S_final_SW=S3S_sw[,.(iso,year,subsector_L3,sector,sw)]
    ## attribute SW==1 to the missing entries   
    S3S_final_SW=addmissingSW(calibr_demand = calibr_demand, grouping_value = "subsector_L3", df_sw = S3S_final_SW)

    ## save the list consisting of sw_fuel,sw_modes, tech_calibration
    calibration_output=list(S2S3_final_SW=S2S3_final_SW,
