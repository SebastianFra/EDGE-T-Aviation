...
lvl0_GCAMraw <- function(input_folder, GCAM_dir = "GCAM"){
  ...
  tech_output_adj = fread(file.path("C:/Users/franz/Documents/R/Master-Thesis/EDGE-T/Data/inputdata/sources/EDGE-T_standalone/GCAM/tech_output_adj.csv"), skip = 1, sep=";", header = T)
  tech_output_adj = melt(tech_output_adj, measure.vars=6:26, value.name="tech_output_adj", variable.name = "year")
  tech_output_adj[, c("Units", "scenario", "year") := list(NULL, NULL, as.numeric(as.character(year)))]
  tech_output_adj = tech_output_adj[year <= 2020 & !subsector %in% c("road","LDV","bus","4W","2W")]
  tech_output_adj[, technology := ifelse(subsector %in% c("Walk","Cycle"), paste0(subsector,"_tmp_technology"),technology)]
  
  setnames(tech_output_adj, old="sector", new="supplysector")
