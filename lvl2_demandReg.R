...
lvl2_demandReg <- function(tech_output, price_baseline, GDP_POP, REMIND_scenario, smartlifestyle,ICCT_data_I,ICCT_data_D,GDP_country,REMIND2ISO_MAPPING,COVID_scenario){
  ...
  if (smartlifestyle) {
    price_el[iso =="REF" & var %in% c("income_elasticity_pass_lo_L","income_elasticity_pass_lo_B", "income_elasticity_pass_sm"), eps := 0]
    price_el[iso %in% c("OAS", "MEA", "LAM") & var %in% c("income_elasticity_pass_lo_L","income_elasticity_pass_lo_B" ,"income_elasticity_pass_sm"), eps := 0.1]
    price_el[iso %in% c("IND") & var %in% c("income_elasticity_pass_lo_L","income_elasticity_pass_lo_B" ,"income_elasticity_pass_sm"), eps := 0.4]
    price_el[iso %in% c("CHA") & var %in% c("income_elasticity_pass_lo_L","income_elasticity_pass_lo_B", "income_elasticity_pass_sm"), eps := 0.2]
    price_el[iso %in% c("SSA") & var %in% c("income_elasticity_pass_lo_L","income_elasticity_pass_lo_B", "income_elasticity_pass_sm"), eps := 0.5]
    price_el[iso %in% c("EUR", "NEU", "USA", "CAZ", "JPN") & var %in% c("income_elasticity_pass_lo_L","income_elasticity_pass_lo_B" ,"income_elasticity_pass_sm"), eps := 0]

  }
  #here adjust for specific countries
  price_el[iso %in% c("IND", "OAS", "SSA", "MEA") & var %in% c("income_elasticity_pass_lo_L","income_elasticity_pass_lo_B"), eps := 0.25]
  ...
  price_el_int_aviation_B_RPK_adj=approx_dt(dt = price_el_int_aviation_B_RPK_adj, ## database to interpolate
                                            ...
                                            idxcols=c("iso"), ## equivalent of "group_by" in dplyr and "by=.(....)" in data.table
                                            extrapolate = T) ## extrapolate? i.e. min(xdata)<min(unique(dat$year))|max(xdata)>max(unique(dat$year))
  price_el_int_aviation_L_RPK<-merge(price_el_int_aviation_L_RPK[,c(1:11,13)], price_el_int_aviation_L_RPK_adj, by = c("iso","year"),all.x = TRUE)
  price_el_int_aviation_B_RPK<-merge(price_el_int_aviation_B_RPK[,c(1:11,13)], price_el_int_aviation_B_RPK_adj, by = c("iso","year"),all.x = TRUE)
  #Leisure Loop
  ...
  price_el[iso %in% c("SSA", "OAS", "IND") & var %in% c("income_elasticity_pass_lo_L","income_elasticity_pass_lo_B"), eps :=eps*0.25]
  ...
  ICCT_data_I<- ICCT_data_I %>% rename(demand_tot = value)
